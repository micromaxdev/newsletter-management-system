...
const Email = require('../models/emailModel');
const SummarizedEmail = require('../models/summarizedEmailModel');
const GenerationConfigModel = require('../models/generationConfigModel');
const { getGeminiModel } = require('../config/gemini');
const fs = require('fs');
const path = require('path');
const cheerio = require('cheerio');
...
const promptTemplate = JSON.parse(
  fs.readFileSync(path.join(__dirname, '../config/systemPrompt.json'), 'utf-8')
);
...
const SUMMARY_RESPONSE_SCHEMA = {
  ...
  properties: {
    title: {
      type: 'string',
      description: 'A concise, human-readable title for the newsletter or email.'
    },
    summary: {
      type: 'string',
      description: 'A clear summary of the email content.'
    },
    seo: {
      type: 'object',
      properties: {
  ...
  const systemInstruction =
    typeof promptTemplate === 'string'
      ...
      : promptTemplate?.prompt ||
        ...
        `You are an assistant that summarizes newsletter emails into structured JSON.`;

  return `${systemInstruction}

Summarize the following email/newsletter content.
...
const generateSummaryWithGemini = async (content, options = {}) => {
  ...
  const geminiModel = getGeminiModel(selectedModel, generationConfig);
  if (!geminiModel) {
    throw new Error('Gemini model not initialized');
  }
...
const parseAndValidateSummary = (text) => {
  ...
  if (!summarizedResult.seo) {
    summarizedResult.seo = {
      metaTitle: summarizedResult.title,
      metaDescription: summarizedResult.summary.slice(0, 160),
      keywords: []
    };
  }
...
const summarizeEmailContent = async (emailId, options = {}) => {
  try {
    ...
    const cleanedHTML = cleanHTML(
      existingEmail.html || '',
      summarizedResult.title || existingEmail.subject || 'Newsletter',
      existingEmail.date
    );

    await markEmailAsSummarized(existingEmail);

    const summarizedEmailDoc = await saveSummarizedEmail(
      emailId,
      summarizedResult,
      cleanedHTML
    );

    await setApprovalStatus(summarizedEmailDoc._id, false);
...
const bulkSummarizeEmails = async (folderId, tagInput) => {
  ...
  const emailsToSummarize = await Email.find({
    folderId,
    tags: { $all: tagRegexArray },
    isSummarized: { $ne: true },
  });

  console.log(
    `Found ${emailsToSummarize.length} emails to summarize for folder: ${folderId}, tags: ${tags}`
  );
...
const autoBulkSummarize = async () => {
  const configs = await GenerationConfigModel.find();

  for (const config of configs) {
    try {
      console.log(
        `Starting bulk summarization for folder: ${config.folder}, tag: ${config.tag}`
      );
      await bulkSummarizeEmails(config.folder, config.tag);
      console.log(
        `Completed bulk summarization for folder: ${config.folder}, tag: ${config.tag}`
      );
    } catch (error) {
      console.error(
        `Error during bulk summarization for folder: ${config.folder}, tag: ${config.tag}`,
        error
      );
    }
  }
...
const injectContentIntoTemplate = (
  content,
  title = 'Newsletter',
  customDate = null
) => {
...
const cleanHTML = (
  ...
  $cleaned('p').each((i, el) => {
    const text = $cleaned(el).text().toLowerCase().trim();
    if (text.startsWith('hi ') || text.startsWith('dear ')) {
      $cleaned(el).remove();
    }
  });
  ...
  $cleaned('p').each((i, el) => {
    const htmlContent = ($cleaned(el).html() || '').trim();

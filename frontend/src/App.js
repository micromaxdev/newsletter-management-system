...
function App() {
  ...
  const fetchEmails = async () => {
    ...
    try {
      ...
      const response = await axios.get(`/api/emails/saved?${params.toString()}`);
      
      if (response.data.emails) {
        setAllEmails(response.data.emails);
        setDisplayedEmails(response.data.emails); // Ensure this is always up-to-date for modal navigation
      } else {
        setAllEmails([]);
        setDisplayedEmails([]);
      }
  ...
  const fetchCounts = async () => {
    try {
      const response = await axios.get("/api/emails/counts");
      
      if (response.data.counts && response.data.unreadCounts) {
        setEmailCounts(response.data.counts);
        setUnreadCounts(response.data.unreadCounts);
      } else if (response.data.error) {
        console.error("Error from counts API:", response.data.error);
        setError(`Failed to load counts: ${response.data.error}`);
      }
    } catch (error) {
      console.error("Error fetching email counts:", error);
      const errorMessage = error.response?.data?.message || 
  ...
  const syncEmails = async () => {
    ...
    try {
      ...
      console.log("Sync complete:", response.data.categorization || "No categorization data");
      await fetchEmails();
      await fetchCounts();
      setSelectedEmailForModal(null); // Close modal if open after sync
    } catch (error) {
  ...
  const moveEmail = async (emailId, newFolderId) => {
    try {
      const response = await axios.put(`/api/emails/${emailId}/folder`, {
      ...
      const errorMessage = error.response?.data?.error || 
                          error.message || 
                          "Failed to move email due to network error.";
      setError(errorMessage);
    }
  };

...
const AutoSummarizationConfigModal = ({ onClose }) => {
  ...
  const fetchFolders = async () => {
    try {
      ...
      const normalizedFolders = Array.isArray(data)
        ? data.map((folder) => ({
  ...
  const updateConfig = async (id, configData) => {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(errorText || 'Failed to update configuration');
    }
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      if (!configToDelete?._id) {
        throw new Error('Invalid configuration selected for deletion');
      }
  ...
  const getFolderLabel = (folderValue) => {
    const found = folders.find((folder) => folder.value === folderValue);
    return found ? found.label : folderValue;
  };

  const canSubmit =
    formData.folder.trim() !== '' &&
    formData.tag.trim() !== '' &&
    !loadingFolders &&
    !submitting;
  ...
  return (
    ...
    >
          ...
          {loadingConfigs ? (
            ...
            <div>
              <h3 style={{ marginBottom: '1rem' }}>
                Existing Configurations Processed every 8 a.m.
              </h3>
                    ...
                    >
                      ...
                      >
                        <div style={{ flex: 1 }}>
                          ...
                          >
                            <div>
                              <strong>Folder:</strong> {getFolderLabel(config.folder)}
                            </div>
                            <div>
                              <strong>Tag:</strong> {config.tag}
                            </div>
      ...
      {showDeleteConfirmation && configToDelete && (
        <ConfirmationModal
          title="Delete Configuration"
          message={`Are you sure you want to delete the configuration for "${getFolderLabel(
            configToDelete.folder
          )}" folder with "${configToDelete.tag}" tag?`}
          highlightText="This action cannot be undone."
          confirmLabel="Delete"
          cancelLabel="Cancel"
          confirmColor="#dc2626"
